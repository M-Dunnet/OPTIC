...
Dependancies can be installed with the requirements.txt file. To install, first create a virtual environment:

    python3 -m venv venv_OPTIC

To activate the virtual environment use the command navigate to the environement direcotry and use the commmand:

    source venv_OPTIC/bin/activate

With the enivronment active, install the dependancies:
...
### Additonal Scripts. 
Additonal scripts are also included to supplement the OPTIC workflow. 
1. The `move_files.sh` script can move files from different subsets (as determined by hierarchical clustering) into seperate directories.
...
## OPTIC.py and OPTIC_Custom.py.
There are two primary scripts: `OPTIC.py` and `OPTIC_Custom.py`. Both scripts function the same; however, `OPTIC.py` is designed to function directly with MAF files from the TCGA GDC portal, while `OPTIC_Custom.py` is set up to be edited for other file types that may have a slightly different structure.

Lines that need changing in `OPTIC_Custom.py`:
* Line 213: File extension
* Line 248 and 250: Check input file columns align with OPTIC
* Line 270 and 271: Check if blank columns are empty strings or hyphens in the input file; change as necessary.
...
#### Anaylsis Options
    ...
    --hierarchial_clustering # Sets oncoscan to cluster samples by mutation profile. The integer provided sets the number of clusters to analyse
...
#### Mutation matrix
The mutation matrix provides a visual identifaction of which genes are mutated at speciifc frequencies. Samples are represented by columns, and genes across rows. Yellow indictaes at least one mutation in the specifed gene, while blue indicates a wild-type genotype.


#### Cumulative coverage plots
The cumulative coverage plot shows the proportion of samples encapsulated by the addition of different genes. Genes are ordered by total mutation frequecy, or, if the set coverage algorythim is used, by total sample coverage increase.
...
Tab-deliminated list showing the gene name (HUGO nomenclature) and the overall mutation frequenicy across all samples. 

#### Variant counts file
Tab-deliminated list showing the gene name (HUGO nomenclature), specific variant (e.g. c.G12A), and the total number of times it has occured accross all samples. 

#### Variant types file
Tab-deliminated list showing the gene name (HUGO nomenclature), the type of mutation (e.g. missense), and the total number of times it has occured accross all samples. 


## Generate_filter.py
Generates a filter file for OPTIC using the COSMIC Cancer Mutation Census dataset. Only variants within the fitler file will be considered by OPTIC.
...
## Additonal scripts.py
### move_files.sh
Running OPTIC with the hierarchial_clustering will produce a TSV format file showing which samples are in each cluster. `move_files.sh` is a short bash script that can take this file as input, and will move files from an original dirctory into cluster-specific directories. For example, if the orignal files are in a dir:
